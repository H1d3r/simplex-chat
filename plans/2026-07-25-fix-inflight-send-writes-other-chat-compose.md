...
applied. All platforms: Android and desktop (`commonMain/ComposeView.kt`)
and iOS (`Shared/Views/Chat/ComposeMessage/ComposeView.swift`).
...
- `Utils.kt:43-46` — `withLongRunningApi` launches on
  ...
  (`ComposeView.kt:967-971`) uses it. Leaving the chat never cancels an
  in-flight send.

Two writes then act on the wrong chat:

- **On the chat switch** — `ComposeView.kt:1338-1342`: the `cs.inProgress`
  ...
  `ComposeView.kt:1343-1353` (`!cs.empty`) then saved them as *that*
  chat's draft on the next switch. Symptom 1.
- **When the send completes** — `ComposeView.kt:942-948`, running in the
  ...
  (`ComposeView.kt:727`) **keeps `contextItem`, i.e. the reply**. On
...
(`ComposeView.kt:1338-1342`). On switching away with a send in flight the
...
sent** (`ComposeView.kt:936-963`). The predicate is shared with the other
senders (`ComposeView.kt:598-600`):
...
`progressByTimeout` at `ComposeView.kt:1591-1598`), reset when switching
...
chat's draft otherwise, so it is never dropped and never appears in
another chat. Staying in the chat is unaffected: the guard is true there
...
- Live message sends (`live`, or `cs.liveMessage != null` for the send
  that finalises a live message when leaving the chat, `ComposeView.kt:1333-1337`).
  ...
  `composeState` inside the coroutine (`ComposeView.kt:682`), so that
...
(`ComposeView.kt:602-614`, `616-634`, `657-679`: member contact
...
iOS has the same architecture and the same defect: `ChatView.swift:34`
holds `@State composeState` shared between chats, `ChatView.swift:355-368`
...
(`ComposeView.swift:682-712`) already clears the previous chat's draft
...
(`ChatView.swift:361-369`), mirroring what `onDisappear` does for the
same case:
...
Interaction with `nd/fix-support-chat-draft` (pending): that branch makes
draft ids scope-aware via `draftChatId(chatId, scope)`. The draft write
added here uses `chat.id` like every other draft write in this file, so it
needs the same treatment when the two are merged; the rest of this change
is orthogonal to it.
